# MOC — This Week

Hand-maintained focus file. Claude reads this first every session.

Week: 2026-W19 (May 4-8)

## Top 3 to push
1. **People Inc** — $150K. ZoomInfo enrich CIO/CISO on Monday. Book a call this week. Slipped four consecutive days in W18. Single-contact death pattern is actively forming. Does not slip again.
2. **Starboard Group** — $50K, POC, May 31 close. File Apr 30 debrief. Add Jose Betancourt as contact 2. Confirm Robert Gunderson's budget authority. 27 days to close, MEDDPICC blank.
3. **AKUVO** — $50K. May 21 POC scoping call. Prep starts this week. William Coffey (CRDO) must be on that call. If Dan McNamara is the only attendee, the deal is not progressing.

_(Medusind: post-settlement urgency window closing. Bala Iyer / John DeJager multi-thread is the play. Does not slip W19.)_
_(Rocket Companies: 7 days post-discovery call, no deal in HubSpot. Create deal, file Apr 24 debrief, draft Chris Burrows. May 7 earnings = timing hook.)_
_(Mitsubishi Heavy: MNDA sent Apr 20, 11+ days dark. Send a follow-up or flag as stalled.)_

## Top 3 new conversations to open
1. **Rocket Companies** — Create HubSpot deal. File Apr 24 debrief. Draft Chris Burrows (CISO). May 7 earnings is the timing hook. 7 days post-call and counting.
2. **JRM / Justin Hershkowitz** — Respond to "do you have any VARs in mind?" (Apr 29). Miles reads manually. Buying signal. Do not route to a partner.
3. **Elevate Textiles / Rick Scot** — Discovery meeting held May 1. Deal created ($50K, Sep 30 close). Debrief to file Monday. Send scoping call request before the Execweb intro window closes.

## One play to run this week
[[40_Plays/sivan-peer-intro]] on Medusind second thread. Target: Bala Iyer (CIO) or John DeJager (VP Technology). The $5M settlement payout (Apr 10) is the trigger. Jigar Shah alone is fragile. This is the highest-ROI move in the pipeline right now.

## Upcoming
- **May 7:** Rocket Companies Q1 earnings. Timing hook for Chris Burrows outreach.
- **May 13:** Onyxia & Meyer (Hold). No vault file for Meyer — create 10_Accounts/Meyer.md before this meeting.
- **May 19:** NYC Cyber Leadership Dinner at Becco. Confirmed RSVPs: Justin Hershkowitz (JRM), Raymond Lipps (Citrix), Stanley Garcia (Madison Logic), Rich Warner, Ashraf Gad, Eric Karpman, Diana (Premier Quantum), Gilad Globen (American Exchange Group), Michael Furman (STB Cyber Risk). Delivery failures: patrice.finamore@idt.com, cynthia.coronel@hackensackmeridian.org, sue_liddie@avoncompany.com, sreddy@avisbudgetgroup.com — backup contacts needed.
- **May 21:** AKUVO POC scoping call (11am ET). Sivan + Brad on invite.
- **May 21:** Onyxia @ Yankee Stadium.
- Fri 4pm: weekly review → `60_Lessons/2026-W19-review.md`.

## 26-draft backlog (carry from W18)
26 Gmail drafts have had zero reactions since Apr 21-22. Oldest: OCC/Jessica Kruse (Apr 22), Medusind/Megan Marshall (Apr 21), 9 healthcare drafts (Apr 21), CI+DIB drafts (Apr 22). Miles should triage these or signal that they need revision. No outbound landed in W18.

## Deals requiring attention in next 7 days
```dataview
TABLE stage, amount, next_action_date
FROM "10_Accounts"
WHERE contains(tags, "active") AND next_action_date <= date(today) + dur(7 days)
SORT next_action_date ASC
```
